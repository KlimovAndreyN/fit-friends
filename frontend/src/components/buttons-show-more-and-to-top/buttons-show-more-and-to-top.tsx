...
function ButtonsShowMoreAndToTop({ divClassNamePrefix, isHaveMoreData, disabled, onShowMoreClick }: ButtonsShowMoreAndToTopProps): JSX.Element {
  ...
  return (
    <div className={`show-more ${divClassNamePrefix}__show-more`}>
      <button className={showMoreButtonClassName} type="button" disabled={!isHaveMoreData || disabled} onClick={handleShowMoreButtonClick}>Показать еще</button>
      <button className={toTopButtonClassName} type="button" disabled={isHaveMoreData || disabled} onClick={handleToTopButtonClick}>Вернуться в начало</button>
    </div>
  );
}
